...
```{r}
cd_data <- read_csv("../../data/circular_dichroism/all_data.csv") %>% 
  janitor::clean_names() %>% 
...
```{r}
annotations <- read_csv("../../data/circular_dichroism/sampleannotations.csv") %>% print()
```
...
cd_data %>% 
  ...
  group_by(sample) %>% 
  nest() %>% 
  mutate(temp = map2(data, sample, function(df, sample) write_tsv(df, paste0("../../data/circular_dichroism/spectrum_files/", sample, ".cd.txt"), col_names = F)))
```
...
ggsave("cd_spectra.pdf")
```
# Plot SESCA inferred values
```{r}
sesca_data <- read_csv("../../data/circular_dichroism/sesca_inferred_ss.csv") %>% 
  left_join(annotations, by = "sample") %>% 
  print()
```
